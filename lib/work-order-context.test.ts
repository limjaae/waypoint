...
describe("buildWorkOrderContext", () => {
  ...
  it("returns null for an unknown work order", async () => {
    expect(await buildWorkOrderContext("wo-does-not-exist")).toBeNull();
  });

  it("pulls asset, location, maintenance history and nearby crews for a known work order", async () => {
    const context = await buildWorkOrderContext("wo-1048");
  ...
  it("surfaces related work orders that share a location", async () => {
  ...
  it("does not include the work order itself in its own related list", async () => {
    const context = await buildWorkOrderContext("wo-1048");
